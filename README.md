...
- **`poke-the-page.html`** — *Poke the Page*: React events, explained with big pictures and few words. Covers event names, `onClick` handlers, the classic `onClick={fn}` vs `onClick={fn()}` bug, the synthetic event, event delegation, and the redraw loop. No dependencies — open it directly in a browser.
